...
// 2. Daten für das Berufswahl-Quiz (Aus Ihrer Legacy-Datei übernommen)
const berufeDB = {
    technik: [
...
// --- MODUL 1: BILDUNGSWEGE WIZARD (Bestehender Code) ---
...
function selectBildungsstand(stand) {
    ...
    // Schritt 1 ausblenden, Schritt 2 anzeigen
    document.getElementById('wizard-step-1').style.display = 'none';
    document.getElementById('wizard-step-2').style.display = 'block';

    // Optionen für Schritt 2 laden
    const zielContainer = document.getElementById('ziel-options');
    zielContainer.innerHTML = ''; // Reset
...
// --- MODUL 2: PDF DOWNLOAD (Bestehender Code) ---
function generatePDF(type) {
    ...
    if(type === 'vabo') {
        ...
        doc.text("Name: _____________________________", 20, 70);
        doc.text("Geburtsdatum: ______________________", 20, 80);
        ...
        doc.text("Schülerdaten: _______________________", 20, 70);
        doc.save("AVdual-Vermittlung.pdf");
    }
}

// --- MODUL 3: GLOSSAR / BEGRIFFE ERKLÄRT (NEU) ---
function initGlossary() {
    const container = document.getElementById('glossar-container');
    const terms = [];

    // Alle Begriffe aus bildungswegeData sammeln
    ...
    terms.forEach((term, index) => {
        ...
        item.className = 'accordion-item glossary-term'; // Klasse für Suche
        item.innerHTML = `
...
// --- MODUL 4: BERUFSWAHL-QUIZ (NEU - Namespaced) ---
const QuizApp = {
    questions: [
        {
    ...
    showResults: function() {
        ...
        // Einfache Logik basierend auf der ersten Antwort (Hauptkategorie)
        const mainCategory = this.answers[0]; 

        // Fallback, falls Kategorie nicht direkt matcht
